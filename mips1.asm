...
# 1 UPLOAPD PREVIOUS POSITION
  lw $a2, 0($s7) # MOVE X       
  lw $a3, 4($s7) # MOVE Y
  add $a2, $t3, $zero # SUBSTRACT MINUS ONE AT THE
...
# VERIFY ONE CONDTION WHEN MOVE X OR Y OR LEFT OR RIGHT
  ...
  beq $t9, $zero, velocity_interruption  # KEEP LOOPING IF IS NOT READY
...
keep_moving:
# THE FUNCITON TAKE THE P 
# S7 I HAVE THE POSITION OF P
# AND SETTING POSITION BY ADDING 
# ALWAYS PLUS BASE ON THE POSITION
# STORE THE POSITION
# WHEN THERE IS AN INSERT CONTINUE 
# WITH THE NORMAL PACE
  # li $a1, 1 # THIS CAN BE X OR  ENTER KEEP MOVING
  move $a0, $s6
# 1 UPLOAPD PREVIOUS POSITION
  lw $a2, 0($s7) # MOVE X       
  lw $a3, 4($s7) # MOVE Y
